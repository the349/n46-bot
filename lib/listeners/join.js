...
class JoinListener extends Listener {
  ...
  exec (member) {
    ...
    // Alert greeters of returning member
    const hasJoinedBefore = this.db.get('JoinListener.logs.' + member.user.id);
    if (hasJoinedBefore) {
      greeters.send(this.client.embed()
        .setColor('#FF851B')
        .addField('This Is A Returning User', 'Treat with care.'));
    }

    this.db.set('JoinListener.logs.' + member.user.id, true);
  }
}
