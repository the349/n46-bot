...
class MessageListener extends Listener {
  ...
  exec (message) {
    ...
    Object.keys(rankgroups).forEach((rankgroup) => {
      let possibleRanks = rankgroups[rankgroup].requirements
        .reduce((possibleRanks, requirement, index) => {
          if (requirement !== '0' && parseInt(requirement) <= xp[userId]) {
            possibleRanks.push(rankgroups[rankgroup].roles[index]);
          }
          return possibleRanks;
        }, []);

      // Sort the ranks by requirement
      possibleRanks = possibleRanks.sort((a, b) => {
        return parseInt(a.requirement) - parseInt(b.requirement);
      }).reverse();

      // Give the new rank if there is one
      if (possibleRanks.length > 0) {
        const newRank = message.guild.roles.find('calculatedPosition', possibleRanks[0].position);
