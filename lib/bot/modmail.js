...
class ModMail {
  ...
  async start (message) {
    ...
    if (!thread.relayChannel) {
      ...
      thread.relayChannel = await thread.relayChannel.overwritePermissions(this.guild.roles.find('name', '@everyone'), {
        VIEW_CHANNEL: false
      });

      thread.relayChannel = await thread.relayChannel.overwritePermissions(this.guild.roles.find('name', 'Moderators'), {
        VIEW_CHANNEL: true,
        SEND_MESSAGES: true,
