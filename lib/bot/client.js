...
class N46Client extends AkairoClient {
  ...
  cooldown (name, username, time) {
    ...
    let user = this.sessiondb.cooldowns.get(username);
    if (!user) user = new Collection();
    ...
    cooldown.run = () => {
      if (user.has(name)) {
        ...
        user.set(name, setTimeout(() => {
          user.delete(name);
        }, time));
      }
    };

    this.sessiondb.cooldowns.set(username, user);
    return cooldown;
  }
