...
class N46Client extends AkairoClient {
    ...
    guild.rolegroups = guild.roles.reduce((rolegroups, role) => {
      if (roleHeaderFormat.test(role.name)) {
        ...
        rolegroups.set(name, {
          ...
          requirements: role.name.replace(roleHeaderFormat, '$3').split(':').slice(0, -1).map(parseInt)
        });
      }
  ...
  updateRoles (guild) {
    ...
    guild.rolegroups = guild.roles.reduce((rolegroups, role) => {
      ...
      if (rolegroup) {
        if (rolegroup.rankgroup) {
          role.requirement = rolegroup.requirements[rolegroup.position - role.calculatedPosition + 1];
        }
    ...
    logger.log('updateRoleGroups', `Roles updated for ${guild.name}`);
  }
}
