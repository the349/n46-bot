  ...
  guild.rolegroups = guild.roles.reduce((rolegroups, role) => {
    ...
    if (rolegroup) {
      if (rolegroup.rankgroup) {
        console.log(role.name, rolegroup.position - role.calculatedPosition - 2);
        role.requirement = parseInt(rolegroup.requirements[rolegroup.position - role.calculatedPosition - 1]);
      }
