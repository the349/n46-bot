...
class UnmuteCommand extends Command {
  ...
  unmute (message, server, scope, member) {
    if (server) {
  ...
  exec (message, { server, scope, member, milliseconds, seconds, minutes, hours, days }) {
    ...
    if ([milliseconds, seconds, minutes, hours, days]
      ...
      return this.unmute(message, server, scope, member);
    }

    return setTimeout(() => {
      message.reply();
      this.unmute(message, server, scope, member);
    }, this.computeTime({ milliseconds, seconds, minutes, hours, days }));
  }
}
